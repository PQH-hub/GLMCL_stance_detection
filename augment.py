...
def synonym_replacement(words, n, raw_words):
    ...
    for random_word in random_word_list:
        ...
        if len(synonyms) >= 1:
            ...
            new_words = [re_words[random_word] if word==random_word else word for word in new_words]#将列表中与 random_word 相等的单词替换为预先定义好的替换值
            num_replaced += 1
...
def get_synonyms(word):#返回给定单词的同义词列表，不包括本身
...
def swap_word(new_words, length):#列表中任意两个位置上的元素值进行交换操作
...
def data_eda(x):
    ...
    for _ in range(num_new_per_technique):
        ...
        # eda_index = 3
        if eda_index == 0:
            a_words = synonym_replacement(words, n_sr, raw_words)#对一段文本中的单词，使用同义词随机替换几个词
        elif eda_index == 1:
            a_words = random_deletion(p_rd, raw_words, x_len)#随机删除单词
        elif eda_index == 2:
            a_words = random_swap(raw_words, n_rs, x_len)#随机交换元素位置
        elif eda_index == 3:
            a_words = random_insertion(raw_words, n_ri, x_len)#随机选择几个词，寻找他们的同义词，然后随机插入
...
if __name__ == "__main__":
    ...
    parser.add_argument('-train', '--train_data', help='Name of the training data file', default='../../data/raw_train_all_onecol.csv',required=False)
    ...
    aug_text.to_csv("../../data/raw_train_all_subset_eda2_kg_onecol.csv", index=False)
